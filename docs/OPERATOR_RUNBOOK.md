...
- Google OIDC can be enabled on top of platform-user auth with `GOOGLE_OIDC_CLIENT_ID`, `GOOGLE_OIDC_CLIENT_SECRET`, and `GOOGLE_OIDC_REDIRECT_URI`. All three are required together (any partial subset fails startup), `USER_STORE_PATH` must also be set (OIDC layers on platform-user auth), and the redirect URI's PATH component must be exactly `/api/auth/oidc/google/callback` (the server's registered callback handler) — mirror that path in the Google Cloud Console's Authorized Redirect URIs. The Cloud Console OAuth client ALSO needs the OAuth scopes `openid`, `email`, and `profile` enabled -- a missing `email` scope is the classic silent-failure config: the consent screen renders fine, but the callback fails with `auth.oidc.failure ... reason=Google%20did%20not%20return%20a%20verified%20email%20address%20for%20this%20account` because the userinfo response carries no `email` claim. Check the scope list FIRST when triaging that pattern. On non-loopback binds, the redirect URI must use `https://...` unless you explicitly set the same insecure-user-auth override for trusted private-network testing. The server requests `access_type=online` (no refresh tokens), so users staying signed in across long absences rely on `USER_AUTH_SESSION_TTL_MS` (default 12h from LOGIN TIME -- the session cookie's `Max-Age` is fixed at login and NOT refreshed by subsequent activity, so a user is auto-signed-out at exactly `loginTime + ttlMs` regardless of how active they were in the interim; see the deployment doc's `USER_AUTH_SESSION_TTL_MS` bullet for the two-layer record-slides-but-cookie-doesn't mechanic), not silent Google refresh -- explains the "why does the user have to re-sign-in every morning" complaint and tells you which knob to raise if a longer-stay UX matters more than the post-leak window. Support triage: if an existing local-password user signs in via Google with the same email, the OIDC flow returns `?auth_error=an account with that email already exists; sign in with its existing method` (deliberate account-hijack defense). Resolution paths are "user signs in with their existing password" or "operator removes the local-password record from `USER_STORE_PATH` and lets OIDC mint a fresh user" — see `docs/HAND_HISTORY_WEB_DEPLOYMENT.md` for the full rationale.
- **OIDC outbound HTTPS egress requirement.** The server makes outbound HTTPS calls to `oauth2.googleapis.com:443` (Google's token endpoint) and `openidconnect.googleapis.com:443` (Google's userinfo endpoint) on every OIDC `/callback`. In a firewalled deployment (corporate egress, container with strict NetworkPolicy, host-level outbound firewall), BOTH endpoints must be reachable from the server -- otherwise OIDC fails at the corresponding step with `auth.oidc.failure ... reason=Google%20OIDC%20exchange%20failed:%20<connect-timeout-or-DNS-error>` after the 5-second JDK `HttpClient` connect ceiling fires. The failure shape matches the catch-all branch of the 13-string finishOidc Left set, with the upstream-error suffix typically a `HttpConnectTimeoutException`. If egress to both Google domains can't be granted in your environment, OIDC sign-in won't work -- fall back to local-password auth (`USER_AUTH_ALLOW_REGISTRATION=true` with `USER_STORE_PATH` configured) or basic auth (`BASIC_AUTH_USER` / `BASIC_AUTH_PASSWORD`). See `docs/HAND_HISTORY_WEB_DEPLOYMENT.md` for the full discussion including the server-side-vs-client-side egress distinction (the user's browser also talks to Google during `/start` redirect + return-from-Google `/callback` redirect, and a user-side network block fails BEFORE reaching our server with no `auth.oidc.failure` log entry at all -- the same shape as the OIDC silent-failure troubleshooting entry in section 6 below).
...
- Auth events emit structured log lines at INFO (success/expected) and WARN (failure). **Local-auth events** carry `email=` (canonical for success, submitted for failure) plus `remote=` (always) plus `reason=` on failures: `auth.login.success email=… remote=…`, `auth.login.failure email=… remote=… reason=…`, `auth.register.success email=… remote=…`, `auth.register.failure email=… remote=… reason=…`, `auth.logout email=… remote=…`. **OIDC events** carry `provider=<id>` (always, so an operator triaging a future multi-provider deployment can filter Google-flow failures from other-IdP failures) plus `remote=` plus an optional `email=` on success only -- the OIDC flow doesn't resolve a user email until the callback userinfo step completes, so the start / start-failure / callback-failure paths emit NO email: `auth.oidc.start provider=… remote=…`, `auth.oidc.start.failure provider=… remote=… reason=…`, `auth.oidc.success provider=… email=… remote=…`, `auth.oidc.failure provider=… remote=… reason=…`. Tail `logs/*.stdout.log` / `logs/*.stderr.log` (or pipe to your log aggregator) and alert on a high WARN rate from a single `remote=` to spot brute-force attempts (or from a known `email=` across many `remote=` sources to spot single-account credential-stuffing). Success lines emit the **canonical** (normalized) email from the stored record so all events for one user grep identically; failure lines emit the **submitted** email so unusual case/whitespace probes are visible in the form the attacker typed. The `auth.oidc.start` entry lets you correlate a click on "Sign in with Google" with the eventual `auth.oidc.success` / `auth.oidc.failure` callback — a stream of starts with no callbacks usually means the provider redirect is misconfigured or a script is exercising the start path.
...
- A successful OIDC `/callback` automatically revokes any pre-existing session whose cookie was present on the same request, so a user who re-authenticates via OIDC while still signed in (e.g. clicked "Sign in with Google" while a previous session was alive) ends with exactly one valid session. Operationally this means a stolen older session token stops working the moment the user re-authenticates — useful when triaging an incident where you suspect a session was exposed: ask the user to sign in again and any leaked-but-pre-existing token is invalidated immediately rather than waiting for the token to expire on its own (which the operator can NOT rely on as a bounded fallback -- a browser-side cookie would expire at `loginTime + USER_AUTH_SESSION_TTL_MS` default 12h FIXED from login time, but an attacker isn't bound by browser cookie semantics: a `curl` / scripted client sending the leaked token directly in a `Cookie:` header bypasses cookie expiry entirely, and each successful request refreshes the SERVER-SIDE record via `resolveSession` so the record stays alive indefinitely past the 12h window as long as the attacker keeps making requests; see the deploy doc's `USER_AUTH_SESSION_TTL_MS` bullet for the record-slides-but-cookie-doesn't mechanic). Re-auth via OIDC is the only reliable upper bound on a leaked-token's lifetime under platform-user auth -- the natural-expiry path requires the attacker to stop using the token for the full ttlMs window (which a determined attacker won't), AND **local-password re-auth doesn't help either**: `handleAuthLogin` returns `409 "already signed in"` when the request carries a valid session cookie, so the legitimate user can't mint a fresh session via the auth form while the leaked-but-still-valid token keeps the old session alive. The OIDC `/callback` handler is the ONLY path that revokes a pre-existing session as a side-effect of the legitimate user re-authenticating. Triage decision matrix: deployment has OIDC configured → ask the user to "Sign in with Google" again; deployment is local-password-only → the user has to either sign out first (which they may not be able to do if they've lost the device that originally signed in) OR the operator restarts the server (the nuclear option that drops EVERY in-memory session record and signs out all current users). See the deploy doc's `USER_AUTH_SESSION_TTL_MS` bullet for the full three-revocation-paths discussion.
...
- Unauthenticated requests to protected paths log as `request unauthorized path=… remote=… reason=…` at WARN. The reason names the auth-stage failure (`missing_authorization`, `unsupported_authorization_scheme`, `malformed_authorization`, `invalid_credentials`, or `session-missing-or-invalid`). The first four come from the Basic-auth path (no `Authorization:` header / header present but not `Basic` / undecodable base64 / decoded fine but `user:password` compare failed -- the compare is constant-time via the non-short-circuit `&` operator, so a timing probe cannot distinguish wrong-username from right-username-wrong-password). The fifth (`session-missing-or-invalid`) comes from the platform-user-auth path and has two distinct underlying causes worth distinguishing: (a) **no cookie on the request** -- typically the session cookie's `Max-Age` expired client-side at exactly `loginTime + USER_AUTH_SESSION_TTL_MS` (default 12h, FIXED from login time regardless of subsequent activity -- this is by far the most common cause for active users; see the deployment doc's `USER_AUTH_SESSION_TTL_MS` bullet for the record-slides-but-cookie-doesn't mechanic), OR the user cleared cookies / closed a private-browsing window; (b) **cookie present but the token doesn't resolve** -- the server-side record expired because the user was idle past ttlMs without any auth request, OR got revoked by an explicit logout in a sibling tab, OR was dropped by a server restart. Both subcases surface as the same `reason=session-missing-or-invalid` -- the log line doesn't distinguish them, but the operator can correlate against an `auth.logout` entry from the same user (sibling-tab logout) or against the `shutdown requested` log line (server restart) to narrow down. Two operator-facing alert patterns: a high WARN rate of `reason=invalid_credentials` from one `remote=` is the credential-stuffing-against-Basic signal; the same shape with `reason=session-missing-or-invalid` for platform-user is a cookie-replay / leaked-session probe.
...
- `SHUTDOWN_GRACE_MS` is the **total** wall-clock budget for graceful shutdown on SIGTERM / Ctrl-C / JVM shutdown hook (default `5000`, i.e. 5 sec). The budget composes across three sequential drain phases under a SINGLE deadline (computed once at shutdown start, NOT a fresh budget per phase): (1) JDK `HttpServer` drains in-flight HTTP requests via `server.stop(httpDrainSeconds)`; (2) the analysis-timeout executor winds down; (3) the analysis worker pool winds down. Each phase claims the wall-time REMAINING vs the shared deadline, so a fast first phase leaves more for the next -- if HTTP requests drain in 100 ms, the analysis executor still gets ~4.9 s to finish in-flight jobs. The HTTP listener drains in whole-second steps (sub-second values round up; the `shutdown requested` log line emits `httpDrainSeconds=<N>` so you can verify the actual value passed). **Exceeded budget**: JVM force-exits, in-flight analyze + hall jobs are lost (pending `Runnable`s dropped, HTTP server stops listening immediately). **Sizing guidance**: raise this knob proportionally with `ANALYSIS_TIMEOUT_MS` / `PLAYING_HALL_TIMEOUT_MS` if those are tuned well above default -- a 5 s grace window assumes typical sub-second analyze completion and accepts the risk of mid-flight hall loss at the 15-min default. For long analyze jobs (multi-minute) or long hall runs, raise grace to comparable scale OR rely on `DRAIN_SIGNAL_FILE` pre-staging (which lets in-flight jobs complete naturally before SIGTERM, dodging the budget question entirely -- see the drain-file bullet below). See `docs/HAND_HISTORY_WEB_DEPLOYMENT.md` for the full discussion.
...
- Use `-DrainSignalFile <path>` or `DRAIN_SIGNAL_FILE=<path>` when you want external deployment tooling to mark the instance unready before shutdown. The check is `Files.exists(path)` on every readiness probe -- lazy, stateless, no filesystem watcher -- so you can `touch <path>` to drain and `rm <path>` to undrain at any time without restarting the server; the next probe reflects the new state. While that file exists, `/api/ready` returns `503` with `reason=draining`, new `POST /api/analyze-hand-history` and `POST /api/playing-hall` submissions are admission-rejected with `503 service is draining`, and `/api/health.drainSignalPresent` flips to `true`. Health checks themselves and in-flight job polling (GET on a still-running `/api/.../jobs/{id}` URL) still work. **In-flight jobs complete naturally** -- drain mode does NOT interrupt them (unlike the `SHUTDOWN_GRACE_MS` budget-exhaustion path which force-drops them on JVM exit overrun). Rolling-restart pattern: (1) `touch` the drain file → reverse proxy / load balancer observes `/api/ready` returning 503 and stops routing new traffic; (2) wait for `/api/health.runningJobs` + `queuedJobs` to drop to zero; (3) SIGTERM the process → the `SHUTDOWN_GRACE_MS` budget covers the residual HTTP-drain + executor-wind-down phases; (4) bring up the new instance, ensuring the drain file is absent. `/api/health.drainSignalConfigured` reflects whether the knob is set (independent of whether the file currently exists), so a dashboard can verify the deployment is wired for graceful rolling restarts at all. Without this knob configured, the only drain signal is the JVM shutdown hook itself -- which doesn't pre-stage the load balancer's stop-routing decision, so the worker pool runs at full traffic right up to the SIGTERM moment. See `docs/HAND_HISTORY_WEB_DEPLOYMENT.md` for the full discussion.
...
- Under platform-user auth, account/profile data persists in `USER_STORE_PATH`, but browser sessions and in-flight review jobs remain in-memory only. A restart signs users out and drops queued/running jobs. **Restart-impact sizing**: check `/api/health.userAuthActiveSessions` BEFORE a planned restart -- every active session triggers a re-auth on the user's next request, hitting `/api/auth/login` within seconds-to-minutes of the restart. If that count was high pre-restart AND your users share a NAT'd egress IP (common in a home / small-office setup), the post-restart re-auth burst can briefly hit the `RATE_LIMIT_AUTH_PER_MINUTE` cap (default 10/IP/min, IP-keyed at the login stage because no user is authenticated yet) and surface as `request rate limited path=/api/auth/login bucket=auth` log lines until the wave clears. Either raise the auth cap temporarily on restart days or warn the user base. Browser-side detail: the session cookie itself stays in the user's browser (cookies are client-side state, not invalidated by the server going down), but its token no longer resolves to any server-side record and falls through to the same not-signed-in path as a never-existed cookie -- the frontend re-prompts via the standard 401 -> reauth path, no special cookie clearing needed. The store JSON contains emails, PBKDF2-HMAC-SHA256 password hashes (per-account salted with 128-bit random salt, 210,000 iterations, 256-bit output -- meets NIST SP 800-132 §5.1 floor of ≥128-bit salt and ≥256-bit output; not plaintext), profile fields (displayName/heroName/preferredSite/timeZone), OIDC subject identifiers, per-account UUIDs (internal `userId`), and event timestamps (`createdAtEpochMs` / `updatedAtEpochMs` / `lastLoginAtEpochMs`). Treat the file as PII: restrict filesystem ACLs to the service account, encrypt backups, and keep it off any tier where unprivileged readers could grep it. A leak is not a credential leak (the hashes resist offline cracking) but the linked emails + OIDC subjects + profile fields are still PII a compliance audit would flag.
...
- The remaining single-value security headers each carry a fixed setting: `Referrer-Policy: no-referrer` (strictest possible -- no `Referer` is ever sent on outbound nav/fetch, one tier stricter than the browser-default `strict-origin-when-cross-origin` which would still leak the origin), `X-Content-Type-Options: nosniff` (forces declared `Content-Type`, closes JS-execution-via-MIME-sniffed-image attack family), `X-Frame-Options: DENY` (no framing, defense in depth alongside CSP `frame-ancestors 'none'`), `Cross-Origin-Opener-Policy: same-origin` (severs cross-origin `window.opener` refs, closes XS-Leaks / Spectre process-isolation surface), `Cross-Origin-Resource-Policy: same-origin` (blocks the app's resources from being embedded by cross-origin documents, additive on top of CORS), and `X-Robots-Tag: noindex, nofollow` (search engines that honor the header skip indexing + URL following; complements the meta tag in index.html). None of these are operator-tunable; a reverse proxy that rewrites or strips them MUST preserve the strict values -- platforms that inject defaults typically use weaker settings (e.g. `Referrer-Policy: strict-origin-when-cross-origin`) that widen the leak surface. See `docs/HAND_HISTORY_WEB_DEPLOYMENT.md` for the full per-header rationale.
...
Users report "Google sign-in fails" but no `auth.oidc.failure` log entries appear:
- The failure happened BEFORE Google redirected back to our `/api/auth/oidc/<provider>/callback`, so the server has no record of the user's attempt -- no log line ever fires for a flow that doesn't complete the redirect roundtrip. Most common cause: the configured `GOOGLE_OIDC_REDIRECT_URI` doesn't match the Authorized Redirect URI in the Google Cloud Console, so Google rejects the OAuth dance at its own consent screen and never sends the user back. Second most common: the Cloud Console OAuth client is in "Testing" state and the user isn't on the Test Users allowlist.
- Triage:
  ```
  curl -s http://<host>:<port>/api/health | jq '{userAuthPendingOidcFlows, userAuthActiveSessions}'
  ```
  `userAuthPendingOidcFlows` counting up steadily (each `/start` writes a state-store entry that times out at the 10-min state TTL without the matching `/callback` ever landing) WITHOUT a corresponding stream of `auth.oidc.success` / `auth.oidc.failure` log lines is the signature of "users start, never finish." Distinguish from the "users finish but fail" case: the latter DOES emit `auth.oidc.failure` lines with a specific `reason=` value (e.g. `reason=Google%20did%20not%20return%20a%20verified%20email%20address%20for%20this%20account` for a missing `email` scope), AND `userAuthPendingOidcFlows` returns to baseline as the failed-callback path consumes its state-store entry.
- Cross-check the configured `GOOGLE_OIDC_REDIRECT_URI` in `conf/hand-history-web.env` against the Google Cloud Console OAuth client's "Authorized redirect URIs" list -- the values must match EXACTLY including scheme (`https` vs `http`), hostname, port (if non-default), and path. The path component is validated server-side at startup and must end with `/api/auth/oidc/google/callback`.
- Also check the Cloud Console OAuth client's enabled scopes: `openid`, `email`, and `profile` are all required. A missing `email` scope is a DIFFERENT failure mode -- the consent screen still renders fine, the callback DOES fire, and the `auth.oidc.failure` line appears with the verified-email reason. So presence vs absence of post-start log entries distinguishes "redirect URI mismatch / Testing-mode rejection" (no logs) from "scope misconfig / consent-screen-completes-but-callback-fails" (logs appear).
- Fix: align the `GOOGLE_OIDC_REDIRECT_URI` env var with the Cloud Console allowlisted redirect URI exactly, then restart the service to pick up the change. If the OAuth client is in Testing mode, either promote it to Production OR add the affected user's email to the Test Users allowlist in the Cloud Console (no service restart needed for the latter). See `docs/HAND_HISTORY_WEB_DEPLOYMENT.md` for the full OIDC-callback failure-mode enumeration.

Users report "rate limited" / "too many requests" / "I'm getting 429 errors":
- The in-process limiter caps the expensive API routes with three independent buckets surfaced in the log line `request rate limited path=… client=… bucket=… limitPerMinute=… retryAfterMs=…`: **submit** (default 6/min, POST `/api/analyze-hand-history` + POST `/api/playing-hall`), **job-status** (default 240/min, GET `/api/.../jobs/{id}`), **auth** (default 10/min, POST `/api/auth/register` + POST `/api/auth/login`). Each bucket has its own cap and the buckets run independently -- a user hitting the auth cap can still poll job-status, and vice versa. 429 response bodies carry three sibling fields (`retryAfterSeconds`, `rateLimitBucket`, `limitPerMinute`) so a scripted client can identify which bucket fired without parsing logs. The cap window is fixed 60-second per client (NOT sliding) so a client can burst up to ~2× the limit at a window boundary -- two-burst-then-throttle is the expected shape, not a bug.
- Triage:
  ```
  grep "request rate limited" logs/*.stdout.log | tail -20
  ```
  Each line names `path=` (which endpoint fired), `client=` (who got throttled), `bucket=` (which cap), `limitPerMinute=` (the current cap value -- compare against the deployment's configured value to spot env-var drift), `retryAfterMs=` (how long the client must wait).
- The `client=` value distinguishes the three legitimate vs adversarial patterns:
  - **`client=user:<userId>`**: a signed-in platform-user is hitting the cap. Usually means the user has too many concurrent tabs open (each tab polls job-status at ~750ms, so >3 tabs at default 240/min eats the bucket steadily) OR they're running a long submit storm. Ask the user to close excess tabs; raise `RATE_LIMIT_STATUS_PER_MINUTE` if they need the parallelism legitimately.
  - **`client=header:<ip>`**: a client behind a trusted reverse-proxy is hitting the cap. The `header:` prefix means `RATE_LIMIT_CLIENT_IP_HEADER` is configured AND the TCP peer is in `RATE_LIMIT_TRUSTED_PROXY_IPS`. Usually a NAT'd small-office / corporate-egress IP shared by several real users. Either raise the per-bucket cap proportionally to your shared-IP user count OR move toward platform-user auth so signed-in users get their own per-user bucket via `client=user:<userId>`.
  - **`client=remote:<addr>`**: a client at the raw TCP peer level is hitting the cap. With no trusted-proxy header configured this is the same as the `header:` case -- shared NAT. With a trusted proxy configured but the peer NOT in `RATE_LIMIT_TRUSTED_PROXY_IPS`, this means the peer is bypassing the proxy or the proxy is misconfigured -- check `rateLimitClientIpSource` in `/api/health` to verify which mode the limiter is in.
- Adversarial pattern: a high WARN rate of `bucket=auth` from one `remote=` with no corresponding `auth.login.success` log lines is the credential-stuffing signature -- the limit is doing its job; do NOT raise the auth cap to "make the warnings stop." For `bucket=submit` and `bucket=job-status` the same all-WARN-no-success pattern is rarer in practice (an attacker isn't submitting valid hand histories) but the same rule applies: legitimate-user-friction warrants raising the cap; adversarial traffic warrants keeping the cap and possibly lowering it.
- Fix matrix: legitimate signed-in user with too many tabs → ask user to close excess OR raise `RATE_LIMIT_STATUS_PER_MINUTE`; legitimate shared-NAT users → raise the offending bucket's cap OR enable trusted-IP-header for per-real-IP keying OR migrate to platform-user auth for per-user keying; adversarial → keep the cap, the limit is working. None of the three changes require a server restart: env-var changes take effect on next process start, but the limiter state itself is in-memory so you can test cap changes by restarting and watching the next rate-limit-WARN line confirm the new `limitPerMinute=` value.

Users report "the page doesn't respond" or "I clicked Sign in / Choose File and nothing happens":
- The page boot path (`site.js`'s `boot()`) wires up DOM handlers, presets, the recent-runs list, auth state, and the dynamic upload cap. If any step throws -- sync from the DOM-setup batch (`mirrorHelpDataToAriaLabel`, `renderPresetBar`, `renderRecentRuns`, `wireHallValidation`, `syncRandomSeedState`) or async from the network probes (`refreshAuthState`, `probeServerLimits`) -- the catch handler reveals a visible `#page-init-error` banner above the site header reading **"This page didn't finish loading."** A visible banner in the user's browser is the signature of "JS executed but threw mid-boot"; an identical symptom WITHOUT the banner means a different failure (HTML stale / banner element absent, JS bundle 404 or CSP-blocked, or the browser predates `boot()` itself running -- fall through to the `STATIC_DIR` triage above to confirm the bundle bytes are at least reaching the browser).
- Triage:
  ```
  curl -s http://<host>:<port>/ | grep -i "page-init-error"
  ```
  Confirms the banner element is present in the served HTML (the bundle hasn't been silently regressed). Banner element present in bytes but NOT visible to the user → JS load failed (script 404, CSP block, network drop between HTML and JS); banner element present AND visible → JS ran but threw, ask the user to open the browser's developer console (F12 → Console) and read the line beginning **`SICFUN: page initialization failed`** -- the `showBootError` handler logs that with the full error class + stack.
- The catch handler is mostly defense-in-depth -- the realistic boot()-killer paths are already guarded internally (`readRecentRuns` / `writeRecentRuns` / `clearRecentRuns` wrap localStorage in try/catch so Safari-private-mode SecurityError and Firefox-quota QuotaExceededError get swallowed there before reaching boot(); `refreshAuthState` / `probeServerLimits` each have their own try/catch around `fetchWithTimeout`; the DOM-touching functions all early-return on null element refs). So if the banner DID appear, the underlying cause is almost always one of three patterns the console line will name:
  - **`TypeError: Cannot read properties of null` (or `undefined`) against a DOM-query expression**: HTML/JS deploy skew -- the operator redeployed a partial bundle where one file's contract doesn't match the other's. An element id the JS dereferences without a null-guard moved or got renamed in the HTML but the JS still expects the old name. Fix: redeploy the full bundle so HTML, CSS, and JS versions match (`bin/verify-release-manifest.ps1` confirms manifest integrity).
  - **`TypeError: Failed to fetch` (Chrome) / `NetworkError when attempting to fetch resource` (Firefox)** from inside `refreshAuthState` or `probeServerLimits`: an upstream network failure escaped the internal try/catch via a future-refactor regression OR the CSP `connect-src` directive tightened in a way that blocks the bundle's own `/api/auth/me` / `/api/health` calls (browsers report CSP fetch blocks as TypeError, not SecurityError, per the WHATWG fetch spec). Fix: check `WebResponses.scala`'s `ContentSecurityPolicy` constant against the API paths the bundle actually calls, particularly `connect-src 'self'` coverage; if the CSP looks correct, the try/catch around the failing fetch may have been removed by an unrelated refactor.
  - **Anything else** (a `SecurityError` thrown by a monkey-patched DOM API, an unexpected `TypeError` from a browser bug, an injected throw from an aggressive content-blocker extension): rare, ask the user to try in a clean browser profile / incognito-without-extensions; if the symptom persists, the underlying error class + stack from the console line is the operator's only signal.
- Fix matrix: deploy-skew TypeError → re-extract or redeploy the full bundle. CSP fetch-block TypeError → widen the CSP `connect-src` in source and rebuild OR roll back the most recent CSP change. Browser-bug / extension-injection → user-side (try a clean profile). If the banner element is NOT in the served HTML at all, the deployed bundle predates the boot-error-banner feature -- deployments built before that point silently lack the diagnostic signal but exhibit the same underlying half-broken page when boot() throws; re-deploy from a build that includes the `#page-init-error` element + `boot().catch(showBootError)` wiring in `site.js`.
