...
- Auth events emit structured log lines at INFO (success) and WARN (failure): `auth.login.success email=… remote=…`, `auth.login.failure email=… remote=… reason=…`, `auth.register.success`, `auth.register.failure`, `auth.logout`, `auth.oidc.success`, `auth.oidc.failure`. Tail `logs/*.stdout.log` / `logs/*.stderr.log` (or pipe to your log aggregator) and alert on a high WARN rate from a single `remote=` to spot brute-force attempts.
