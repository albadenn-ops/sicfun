...
- `auth.oidc.failure` lines carry a `reason=` value naming the failure category. Five distinct patterns, from earliest fire-stage to latest: `missing_state_cookie` / `state_cookie_mismatch` (state-binding check failed, see the bullet above); `provider-error:<oauth_error_code>` when the provider sent a `?error=<code>` callback before we got to talk to it (`provider-error:access_denied` is a user-declined sign-in, `provider-error:temporarily_unavailable` is a provider outage, `provider-error:invalid_request` and others are RFC 6749 §5.2 standard codes); `oversize_callback_param` when either the `state` or `code` query param exceeds 256 chars (attacker probing the callback shape rather than legitimate provider traffic); `missing_code_or_state` when one of the two required query params is absent (truncated redirect, browser-side history-modification, etc.); and a variable-content reason drawn from this fixed seven-string finishOidc Left set (all space-bearing, so they grep as `%20`-escaped in the log): `OIDC login state expired or is invalid` (state-store TTL expired or state value never issued/already consumed), `Google token exchange failed with status <N>` (token endpoint non-2xx -- includes PKCE verifier mismatch which Google returns as 400), `Google token exchange did not return an access token` (token endpoint 2xx but `access_token` field missing -- pathological-provider or upstream-tampering signal), `Google userinfo request failed with status <N>` (userinfo endpoint non-2xx after a successful token exchange), `Google did not return a verified email address for this account` (userinfo came back 2xx but `email_verified: false`, so the identity binding is refused), `an account with that email already exists; sign in with its existing method` (OIDC-vs-local-password email collision documented in the user-store section), and `Google OIDC exchange failed: <upstream-message>` (catch-all for exceptions mid-exchange -- network failure, JSON parse error on a malformed provider response, etc.). Triage shorthand: pattern 2 (provider-error:*) is provider-side, the rest are our-side or transport-side. See `docs/HAND_HISTORY_WEB_DEPLOYMENT.md` for the full per-pattern fire-stage discussion.
