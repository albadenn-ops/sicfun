...
- Common error stacks the console will show + their interpretations: `SecurityError` on `localStorage.getItem` (Safari private browsing rejects storage access; user-side fix is to disable private mode for this origin OR accept that the recent-runs panel won't persist across reloads -- not an operator fix); `QuotaExceededError` on localStorage (Firefox storage full; user-side fix is to clear site data); `TypeError: Cannot read properties of null` against a `data-help` or DOM-query path (HTML/JS deploy skew -- the operator redeployed a partial bundle where one file's contract doesn't match the other's, redeploy the full bundle so HTML, CSS, and JS versions match); a `SecurityError` from inside `refreshAuthState` or `probeServerLimits` (the Content-Security-Policy header tightened in a way that blocks the bundle's own resources -- check `WebResponses.scala`'s `ContentSecurityPolicy` constant against the asset URLs the bundle actually loads, particularly `script-src` and `connect-src`).
- Fix matrix: localStorage Security/Quota errors are user-side (no operator action). Deploy skew → re-extract or redeploy the full bundle so manifest hashes match (`bin/verify-release-manifest.ps1` confirms). CSP-block → either widen the CSP in source and rebuild OR roll back the most recent CSP change. If the banner element is NOT in the served HTML at all, the deployed bundle predates the boot-error-banner feature -- deployments built before that point silently lack the diagnostic signal but exhibit the same underlying half-broken page when boot() throws; re-deploy from a build that includes the `#page-init-error` element + `boot().catch(showBootError)` wiring in `site.js`.
