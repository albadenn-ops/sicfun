...
- `USER_AUTH_ALLOW_REGISTRATION` (default `true`): when `false`, `POST /api/auth/register` returns `403` regardless of the cap. Use this for closed deployments where accounts are minted out-of-band (e.g. via an operator-only flow that calls `registerLocal` directly) and the public registration form should be locked. OIDC sign-up is still allowed since it doesn't go through `/api/auth/register`.
