...
if (-not (Test-Path $dotnet)) {
  throw "dotnet not found at $dotnet. Run scripts/gpu/build-g5-acpc.ps1 first. data/tmp/toolchains is local setup and is not source-controlled."
}
if (-not (Test-Path $g5Dll)) {
  throw "G5.Acpc.dll not found at $g5Dll. Run scripts/gpu/build-g5-acpc.ps1 first. The runtime under data/tmp/ is generated locally and is not source-controlled."
}
