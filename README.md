...
- Quick validation (core runtime smoke): `powershell -ExecutionPolicy Bypass -File scripts/validation/prove-pipeline.ps1 -Quick`
- Full validation (includes hand-history review E2E): `powershell -ExecutionPolicy Bypass -File scripts/validation/prove-pipeline.ps1`
- Review local hand histories: `powershell -ExecutionPolicy Bypass -File scripts/packaged-hand-history-web/start-hand-history-web.ps1`
- Deploy the packaged hand-history web product: [docs/HAND_HISTORY_WEB_DEPLOYMENT.md](docs/HAND_HISTORY_WEB_DEPLOYMENT.md)
...
To reduce accidental exposure, the server now refuses non-loopback binds unless you configure `BASIC_AUTH_*`, enable `USER_STORE_PATH`, or explicitly set `ALLOW_UNAUTHENTICATED_PUBLIC_BIND=true` / `--allowUnauthenticatedPublicBind=true` for a trusted private network.
...
On non-loopback binds, platform-user auth now requires `USER_AUTH_COOKIE_SECURE=true` unless you explicitly set `ALLOW_INSECURE_USER_AUTH=true` / `--allowInsecureUserAuth=true` for trusted private-network testing.
Google login is supported through the built-in OIDC provider when `GOOGLE_OIDC_CLIENT_ID`, `GOOGLE_OIDC_CLIENT_SECRET`, and `GOOGLE_OIDC_REDIRECT_URI` are configured alongside `USER_STORE_PATH`. On non-loopback binds, the redirect URI must use `https://` unless you explicitly set the same insecure-user-auth override for trusted private-network testing.
...
That assembles `dist/hand-history-web`, writes `bin/run-hand-history-web.ps1`, validates the expected static assets, and smoke-checks packaged fail-closed non-loopback config rejection, authenticated `GET /`, unauthenticated `GET /api/health`, unauthenticated `GET /api/ready`, async `POST /api/analyze-hand-history`, drain-mode readiness, and oversized-upload rejection from the packaged layout.
The packaged layout also includes a deployment README at `dist/hand-history-web/README.md`, `conf/hand-history-web.env` for runtime settings, `bin/verify-release-manifest.ps1` for integrity checks, plus `bin/install-hand-history-web-service.ps1`, `bin/start-hand-history-web-service.ps1`, `bin/drain-stop-hand-history-web-service.ps1`, and `bin/uninstall-hand-history-web-service.ps1` for a Windows service workflow backed by NSSM.
After copying the bundle to its target machine, run `powershell -ExecutionPolicy Bypass -File dist/hand-history-web/bin/verify-release-manifest.ps1` once to confirm the shipped files still match the generated `manifest.sha256`.
