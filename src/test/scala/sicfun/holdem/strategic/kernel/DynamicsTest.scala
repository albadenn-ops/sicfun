...
class DynamicsTest extends munit.FunSuite:
  ...
  test("same signal under (Attrib,Off) and (Attrib,On) yields different results only via showdown"):
    ...
    val offKernel = KernelConstructor.composeFullKernelForWorldFull(
      actionKernel,
      actionKernel,
      designKernel,
      sdKernel
    )(attribOff)
    val onKernel = KernelConstructor.composeFullKernelForWorldFull(
      actionKernel,
      actionKernel,
      designKernel,
      sdKernel
  ...
  test("counterfactualReferenceWorld with ChainWorld(Ref, On) produces expected result"):
    ...
    val refKernel = KernelConstructor.composeFullKernelForWorldFull(
      actionKernel,
      actionKernel,
      designKernel,
      sdKernel
