...
private[holdem] object HoldemPostflopNativeRuntime:
  ...
  private final case class RuntimeConfig(
      ...
      nativeEngine: String,
      autoMinGpuWork: Long,
      postflopAutoTuneEnabled: Boolean,
      hasExplicitPostflopCudaConfig: Boolean,
      resolvedPostflopAutoTuneCacheFile: File
  )
  ...
  private def postflopAutoTuneEnabled: Boolean =
    runtimeConfig().postflopAutoTuneEnabled
  ...
  private def runtimeConfig(): RuntimeConfig =
    ...
    if cached != null then cached
      ...
      val computed = RuntimeConfig(
        provider = GpuRuntimeSupport.resolveNonEmptyLower(ProviderProperty, ProviderEnv).getOrElse("auto"),
        nativeEngine =
        ...
        postflopAutoTuneEnabled =
          GpuRuntimeSupport.resolveNonEmptyLower(PostflopAutoTuneProperty, PostflopAutoTuneEnv) match
            case Some("0" | "false" | "no" | "off") => false
            case _ => true,
        hasExplicitPostflopCudaConfig =
        ...
        resolvedPostflopAutoTuneCacheFile =
      ...
      runtimeConfigRef.compareAndSet(null, computed)
