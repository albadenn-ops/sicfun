...
object ValidationRunner:
  private val log = ConsoleLogger.fromConfig("validation-runner")
    ...
    for stepIdx <- 0 until totalSteps do
      ...
      profiles.headOption.foreach { profile =>
        ...
        if stepIdx == totalSteps - 1 && !detected then
          ...
          log.debug(f"$villainName: evts=${evts.size} turn(n=${turnEvts.size} r=$turnRaises) rvr(facing=${riverFacing.size} folds=$riverFolds) hints=$hints")
      }
  ...
  def runWebSpotCheck(config: Config, results: Vector[PlayerValidationResult]): Unit =
    ...
    log.info("Starting HandHistoryReviewServer...")
    ...
    HandHistoryReviewServer.start(serverArgs) match
      case Right(server) =>
        log.info("Web server running at http://127.0.0.1:8090/")
        log.info("Upload the chunk files above to analyze in the browser.")
        log.info("Press Ctrl+C to stop.")
        try Thread.currentThread().join()
        catch case _: InterruptedException => ()
        finally server.close()
      case Left(err) =>
        log.error(s"Failed to start web server: $err")
