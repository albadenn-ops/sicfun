...
object HeadsUpBackendComparison:
  ...
  private def parseArgs(args: Vector[String]): Config =
    ...
    Config(
      ...
      maxMatchups = CliHelpers.requireLongOption(options, "maxMatchups", 4000L),
      ...
      nativeCpuEngine = options.getOrElse("nativeCpuEngine", "cpu").trim.toLowerCase,
      nativeGpuEngine = options.getOrElse("nativeGpuEngine", "cuda").trim.toLowerCase,
      nativeCudaBlockSize = CliHelpers.optionalIntOption(options, "nativeCudaBlockSize"),
      nativeCudaMaxChunkMatchups = CliHelpers.optionalIntOption(options, "nativeCudaMaxChunkMatchups"),
      nativeAutoTune = CliHelpers.requireBooleanOption(options, "nativeAutoTune", true)
    )
