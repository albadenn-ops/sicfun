package sicfun.holdem.bench.tuner
...
object HeadsUpRangeGpuAutoTuner:
  ...
  private val AllowedOptionKeys = Set("heroes", "entriesPerHero", "trials", "warmupRuns", "runs", "seedBase", "nativePath", "cachePath")
  ...
  private def parseArgs(args: Vector[String]): Config =
    ...
    Config(
      heroes = CliHelpers.requireIntOption(options, "heroes", 256),
      entriesPerHero = CliHelpers.requireIntOption(options, "entriesPerHero", 128),
      trials = CliHelpers.requireIntOption(options, "trials", 64),
