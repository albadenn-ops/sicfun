...
object HoldemBayesBenchmark:
  ...
  private final class BenchmarkRunner(config: Config):
    private val context = buildContext(config.seed)

    def run(): Unit =
      configureRuntime()
      printHeader()
      warmup()
      printResults(measureSamples())

    private def configureRuntime(): Unit =
    ...
    private def measureSamples(): Vector[Sample] =
    ...
    private def printResults(samples: Vector[Sample]): Unit =
      val byStage = samples.groupBy(_.stage).toVector.sortBy(_._1)
      println()
  ...
  private def parseArgs(args: Vector[String]): Config =
    ...
    Config(
      ...
      bunchingTrials = CliHelpers.requireIntOption(options, "bunchingTrials", 1_200),
      equityTrials = CliHelpers.requireIntOption(options, "equityTrials", 8_000),
      provider = options.getOrElse("provider", "auto"),
      seed = CliHelpers.requireLongOption(options, "seed", 17L)
    )
