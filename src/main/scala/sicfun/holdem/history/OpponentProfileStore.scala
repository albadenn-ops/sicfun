...
/** Aggregated per-action counters used to derive compact behavioral signatures.
  *
  * Tracks fold/raise/call/check counts and cumulative pot-odds-when-calling.
  * The [[signature]] method converts these raw counts into a normalized
  * [[PlayerSignature]] vector suitable for archetype classification and clustering.
  *
  * @param folds            total fold actions observed
  * @param raises           total raise/bet actions observed
  * @param calls            total call actions observed
  * @param checks           total check actions observed
  * @param callPotOddsTotal cumulative pot odds across all call actions (for averaging)
  */
final case class OpponentActionSummary(
  ...
  /** Convert raw counts to a 6-element normalized signature vector:
    * [foldRate, raiseRate, callRate, checkRate, actionEntropy, avgPotOddsWhenCalling].
    */
...
/** Snapshot of profile stability metrics over sliding windows.
  *
  * Used to track how much a player's behavioral signature changes over time.
  * A stable profile (low drift, few changes) indicates consistent opponent play.
  *
  * @param windowCount        number of sliding windows evaluated
  * @param significantChanges count of windows where signature drift exceeded threshold
  * @param maxDrift           maximum drift observed across all windows
  * @param meanDrift          average drift across all windows
  */
...
/** A record of cards revealed at showdown, used for showdown-based profiling.
  *
  * @param handId the hand in which the showdown occurred
  * @param cards  the hole cards revealed
  */
...
/** A structured exploit hint generated by the profiling pipeline.
  *
  * Each hint identifies a specific exploitable pattern (ruleId), provides a
  * human-readable description (text), and includes 4 quantitative metrics:
  *   - metrics(0): deviationRatioFromGto — how far the player deviates from equilibrium
  *   - metrics(1): evLosing — estimated EV lost due to this leak
  *   - metrics(2): leakDetectionConfidence — confidence that this is a real leak (not noise)
  *   - metrics(3): exploitEdgeConfidence — confidence that exploiting this leak is profitable
  *
  * @param ruleId  machine-readable rule identifier (e.g., "overcall-big-bets")
  * @param text    human-readable exploit hint description
  * @param metrics exactly 4 numeric metrics (see above)
  */
...
/** Persisted long-term profile for a remembered opponent.
  *
  * Captures a player's behavioral statistics across all observed hands:
  * action frequencies, raise responses, recent event history, showdown records,
  * and identity fields. The profile supports incremental updates via [[merge]]
  * and provides derived outputs:
  *   - [[signature]] — compact 6-element vector for clustering
  *   - [[archetypePosterior]] — Bayesian posterior over player archetypes (TAG, LAG, etc.)
  *   - [[exploitHintDetails]] — structured exploit hints with confidence metrics
  *   - [[stability]] — drift analysis for detecting style changes over time
  *
  * @param site                  the poker site this profile was collected from
  * @param playerName            player's screen name
  * @param handsObserved         total hands observed
  * @param firstSeenEpochMillis  timestamp of first observation
  * @param lastSeenEpochMillis   timestamp of most recent observation
  * @param actionSummary         aggregated action counts (fold/raise/call/check)
  * @param raiseResponses        how the player responds to hero raises
  * @param recentEvents          sliding window of recent PokerEvents for stability analysis
  * @param seenHandIds           hand IDs seen (for dedup during merge)
  * @param showdownHands         cards revealed at showdown
  * @param playerUid             deterministic player identity UID
  * @param profileUid            deterministic profile identity UID
  * @param behaviorUid           content-addressable behavior fingerprint UID
  * @param modelUid              optional SICFUN model UID (for local players)
  */
