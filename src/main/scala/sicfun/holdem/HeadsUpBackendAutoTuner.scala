    ...
    if !autoTuneEnabled then
      ...
      if !availability.available then
        ...
        if batch.size <= 0 then
          ...
          val cacheFile = resolvedCacheFile
          val signature = workloadSignatureExact(tableKind = tableKind, tuneEntries = batch.size)

          loadDecisionFromCache(cacheFile, signature) match
            case Some(cached) =>
              applyDecision(cached)
              GpuRuntimeSupport.log(
                s"gpu-autotune: exact cached engine=${cached.engine}, block=${showOpt(cached.blockSize)}, " +
                  s"chunk=${showOpt(cached.maxChunkMatchups)} (entries=${batch.size}, ${cached.detail})"
              )
            case None =>
              val exactMode = HeadsUpEquityTable.Mode.Exact
              val results =
                ExactCudaCandidates
                  .filter(cfg => cfg.maxChunkMatchups <= batch.size)
                  .flatMap(cfg =>
              ...
              if results.isEmpty then
                ...
                val winner = results.minBy(_.seconds)
                val decision = Decision(
                  engine = winner.engine,
                  blockSize = winner.blockSize,
                  maxChunkMatchups = winner.maxChunkMatchups,
