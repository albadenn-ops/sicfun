...
object HoldemCfrSolver:
  ...
  private val equityLookupCache = new ConcurrentHashMap[EquityLookupCacheKey, Map[HoleCards, Double]]()
        ...
        if heroActions.forall(_ == PokerAction.Fold) then None
          ...
          Some(
            villainSupport.foldLeft(0.0) { case (acc, (villain, probability)) =>
              acc + (probability * equityByVillain.getOrElse(villain, 0.0))
            }
          )
  ...
  private def maybeSolveDirectShallowPolicy(
    ...
    while idx < heroActions.length do
      directShallowRootActionValue(game, heroActions(idx)) match
        case Some(value) =>
  ...
  private def directShallowRootActionValue(
    ...
    action match
      ...
      case PokerAction.Check =>
        Some(weightedEquity(game) * game.publicState.pot)
      case PokerAction.Call =>
        val equity = weightedEquity(game)
        Some((equity * (game.publicState.pot + game.publicState.toCall)) - game.publicState.toCall)
      case PokerAction.Raise(amount) =>
        directShallowRaiseValue(game, amount)

  private def directShallowRaiseValue(
      game: HoldemDecisionGame,
      heroRaise: Double
  ): Option[Double] =
    val responses = game.villainResponseByRaise.getOrElse(heroRaise, Vector(PokerAction.Fold, PokerAction.Call))
    val reraises = responses.collect { case PokerAction.Raise(amount) => amount }
    val villainCount = game.villainDistribution.length
    ...
    while villainIdx < villainCount do
      val (villain, probability) = game.villainDistribution(villainIdx)
      val equity = game.equityByVillain(villain)
    ...
    while reraiseIdx < reraises.length do
      ...
      val heroResponses = game.heroResponseByReraise.getOrElse((heroRaise, villainRaise), Vector(PokerAction.Fold))
  ...
  private def weightedEquity(game: HoldemDecisionGame): Double =
    game.villainDistribution.foldLeft(0.0) { case (acc, (villain, probability)) =>
      acc + (probability * game.equityByVillain.getOrElse(villain, 0.0))
    }
  ...
  private def trimVillainSupport(
    ...
    val selectedHands = new Array[HoleCards](limit)
    val selectedWeights = new Array[Double](limit)
    val selectedIds = new Array[Int](limit)
    var selectedSize = 0
    ...
    def worseThan(
      ...
      leftWeight < rightWeight || (leftWeight == rightWeight && leftId > rightId)

    def swapEntries(left: Int, right: Int): Unit =
      val hand = selectedHands(left)
      selectedHands(left) = selectedHands(right)
      selectedHands(right) = hand

      val weight = selectedWeights(left)
      selectedWeights(left) = selectedWeights(right)
      selectedWeights(right) = weight

      val id = selectedIds(left)
      selectedIds(left) = selectedIds(right)
      selectedIds(right) = id

    def siftUp(start: Int): Unit =
      var idx = start
      while idx > 0 do
        val parent = (idx - 1) >>> 1
        if worseThan(selectedWeights(idx), selectedIds(idx), selectedWeights(parent), selectedIds(parent)) then
          swapEntries(idx, parent)
          idx = parent
        else idx = 0

    def siftDown(start: Int): Unit =
      var idx = start
      var continue = true
      while continue do
        val left = (idx << 1) + 1
        if left >= selectedSize then continue = false
        else
          val right = left + 1
          var worst = left
          if right < selectedSize &&
              worseThan(selectedWeights(right), selectedIds(right), selectedWeights(left), selectedIds(left))
          then worst = right
          if worseThan(selectedWeights(worst), selectedIds(worst), selectedWeights(idx), selectedIds(idx)) then
            swapEntries(idx, worst)
            idx = worst
          else continue = false

    val iterator = villainPosterior.weights.iterator
    while iterator.hasNext do
      val (hand, weight) = iterator.next()
      val firstMask = cardMask(hand.first)
      val secondMask = cardMask(hand.second)
      if weight > 0.0 && (deadMask & firstMask) == 0L && (deadMask & secondMask) == 0L then
        val handId = holeCardsId(hand)
        if selectedSize < limit then
          selectedHands(selectedSize) = hand
          selectedWeights(selectedSize) = weight
          selectedIds(selectedSize) = handId
          selectedSize += 1
          siftUp(selectedSize - 1)
        else if worseThan(selectedWeights(0), selectedIds(0), weight, handId) then
          selectedHands(0) = hand
          selectedWeights(0) = weight
          selectedIds(0) = handId
          siftDown(0)

    require(selectedSize > 0, "villain posterior is empty after dead-card filtering/capping")
    val selected = ArrayBuffer.empty[(HoleCards, Double, Int)]
    var selectedIdx = 0
    while selectedIdx < selectedSize do
      selected += ((selectedHands(selectedIdx), selectedWeights(selectedIdx), selectedIds(selectedIdx)))
      selectedIdx += 1
    selected.sortInPlaceBy(_._3)

    var total = 0.0
    var idx = 0
    while idx < selected.length do
      total += selected(idx)._2
      idx += 1
    ...
    idx = 0
    while idx < selected.length do
      val (hand, weight, _) = selected(idx)
      builder += hand -> (weight * invTotal)
      idx += 1
    builder.result()
    ...
    HoldemPostflopNativeRuntime.computePostflopBatch(
      ...
      case Right(values) if values.length == villains.length =>
        ...
        while idx < values.length do
          builder += villains(idx) -> values(idx).equity
          idx += 1
        Some(builder.result())
      case _ =>
        None
  ...
  private final case class HoldemDecisionGame(
    ...
    def toNativeTreeSpec: HoldemCfrNativeRuntime.NativeTreeSpec =
      ...
      while villainIdx < villainCount do
        ...
        villainEquities(villainIdx) = equityByVillain.getOrElse(
          villain,
          throw new IllegalStateException(s"missing equity value for villain hand ${villain.toToken}")
        )
        villainIdx += 1
