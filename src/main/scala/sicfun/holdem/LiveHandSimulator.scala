...
object LiveHandSimulator:
  ...
  private def parseArgs(args: Array[String]): Either[String, CliConfig] =
    if args.contains("--help") || args.contains("-h") then Left(usage)
      ...
      for
        ...
        adaptiveObservations <- parseIntOption(options, "adaptiveObservations", 24)
        ...
        decisionBudget <- parseOptionalLongOption(options, "decisionBudgetMillis")
        _ <- decisionBudget match
          case Some(value) if value <= 0L => Left("--decisionBudgetMillis must be > 0 when provided")
          case _ => Right(())
        showTopPosterior <- parseIntOption(options, "showTopPosterior", 5)
        _ <- if showTopPosterior > 0 then Right(()) else Left("--showTopPosterior must be > 0")
        keepArtifacts <- parseBooleanOption(options, "keepArtifacts", false)
      yield CliConfig(
        hero = hero,
        seed = seed,
  ...
  private def parseOptions(args: Array[String]): Either[String, Map[String, String]] =
    ...
    pairs.foldLeft(Right(Map.empty): Either[String, Map[String, String]]) {
      ...
      case (Right(acc), Right((key, value))) => Right(acc.updated(key, value))
    }
    ...
    options.get(key) match
      ...
      case Some(raw) =>
        raw.toIntOption match
          ...
          case None => Left(s"--$key must be an integer")
    ...
    options.get(key) match
      ...
      case Some(raw) =>
        raw.toLongOption match
          case Some(value) => Right(value)
          case None => Left(s"--$key must be a long")

  private def parseOptionalLongOption(
      options: Map[String, String],
      key: String
