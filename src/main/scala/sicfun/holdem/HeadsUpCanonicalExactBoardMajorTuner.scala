  ...
  private def parseArgs(args: Vector[String]): Config =
    args.foldLeft(Config()) { (cfg, raw) =>
      ...
      key match
        ...
        case "chunkBoards" => cfg.copy(chunkBoards = parsePositiveIntList(value, "chunkBoards"))
        case "scoreThreads" => cfg.copy(scoreThreads = parsePositiveIntList(value, "scoreThreads"))
        case "matchThreads" => cfg.copy(matchThreads = parsePositiveIntList(value, "matchThreads"))
        case "prepareBoardsPerBlock" =>
          cfg.copy(prepareBoardsPerBlock = parsePositiveIntList(value, "prepareBoardsPerBlock"))
        case other => throw new IllegalArgumentException(s"unknown option '$other'")
    }

  private def parsePositiveIntList(raw: String, key: String): Vector[Int] =
    val values = raw.split(",").toVector.map(_.trim).filter(_.nonEmpty).map(_.toInt)
    require(values.nonEmpty, s"$key must be non-empty")
    values.foreach(v => require(v > 0, s"$key values must be positive"))
    values.distinct
