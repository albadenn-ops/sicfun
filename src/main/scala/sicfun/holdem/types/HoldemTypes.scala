...
final case class HoleCards(first: Card, second: Card):
  ...
  /** Checks whether this hand contains the given card. Int-based for speed. */
  inline def contains(card: Card): Boolean = first.id == card.id || second.id == card.id

  /** Returns true when the two hands do not share any card. Int-based for speed. */
  inline def isDisjointFrom(other: HoleCards): Boolean =
    first.id != other.first.id &&
      first.id != other.second.id &&
      second.id != other.first.id &&
      second.id != other.second.id

  override def hashCode(): Int = first.id * 52 + second.id
  override def equals(that: Any): Boolean = that match
    case h: HoleCards => first.id == h.first.id && second.id == h.second.id
    case _ => false
